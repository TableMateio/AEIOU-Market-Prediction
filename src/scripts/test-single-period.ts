...
/**
 * Test Single Period Collection
 * Start small - test one 3-day period first
 */
...
class SinglePeriodTester {
    ...
    async testSinglePeriod(): Promise<void> {
        console.log('🧪 TESTING SINGLE PERIOD COLLECTION');
        console.log('='.repeat(60));
        console.log('Goal: Validate strategy with one 3-day period before scaling');
        console.log('');
    ...
    private async checkDatabaseSchema(): Promise<void> {
        ...
        try {
            ...
            if (error && error.message.includes('column')) {
                console.log('   ⚠️  New fields (categories, concepts, social_score) not found');
            ...
            console.log('   ⚠️  Schema check failed, will store in metadata as fallback');
            console.log('');
        }
    }
    ...
    private async applyDatabaseMigration(): Promise<void> {
        try {
            ...
            if (error) {
                console.log('   ❌ Migration failed via RPC, will use fallback storage');
            } else {
                console.log('   ✅ Migration applied successfully');
            }
        } catch (error: any) {
            console.log('   ❌ Migration failed, will use metadata fallback');
        }
        console.log('');
    }
    ...
    private generateTestPeriod(): any {
        ...
        // Let's use a Monday-Wednesday period for good business day coverage
    ...
    private async executeTestCollection(period: any): Promise<void> {
        ...
        try {
            ...
            // Save to database
            await this.saveToDatabase(relevantArticles, period);

            console.log('🎉 SINGLE PERIOD TEST COMPLETED SUCCESSFULLY!');
            console.log('');
            console.log('📋 NEXT STEPS:');
            console.log('   1. Review the saved articles in database');
            console.log('   2. If satisfied, run: npx tsx src/scripts/test-five-periods.ts');
            console.log('   3. Then scale to full year and eventually full 5 years');
     ...
     */
    private async saveToDatabase(articles: any[], period: any): Promise<void> {
        ...
        for (const [index, article] of articles.entries()) {
            try {
                ...
                // Transform article for database
                ...
                // Try to add new fields if available
                try {
                    const articleWithNewFields = {
                        ...transformedArticle,
                    ...
                    inserted++;
                    console.log(`      ✅ Inserted with new fields`);

                } catch (newFieldError: any) {
                    // Fallback: store in metadata if new fields don't exist
                    if (newFieldError.message?.includes('column')) {
                        ...
                        inserted++;
                        console.log(`      ✅ Inserted with metadata fallback`);
                    } else {
                        throw newFieldError;
                    }
                }
...
async function main() {
    try {
        const tester = new SinglePeriodTester();
